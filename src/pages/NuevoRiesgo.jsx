...
const NuevoRiesgo = (props) => {
    ...
    const [amenazaS, setAmenazaS] = useState([{id:1, value:"1"}]);
        ...
        <div className="w-80% m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl" style={{ width: '200em' }}>
          ...
          text-white'
            ...
            <form onSubmit={() => { handleSubmitRiesgo() }}>
                ...
                <label>Amenaza
                    <br />
                    <select onChange={(e)=>setAmenaS(e.target.value)} >
                        {amenazaS.map((amenaza) => (
                            <option key={amenaza.id} value={amenaza.value}>{amenaza.value}</option>
                        ))}
                    </select>
